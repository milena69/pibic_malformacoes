...
import { Buttons, Body, Titulo, ProgressBar, AlertaToastError } from '../../components';
import questionario from '../../assets/questionario.json';
import { ButtonWrapper, DivRespostas, ProgressWrapper, SpanProgress } from './styled';
...
const Home = () => {
	...
	const handleCheckChange = ({ target: { checked } }, idResposta) => {
		...
		if (!arrayQuestionario[step].multiplaEscolha) {
			...
			newRespostas = respostasQuestao.map(({ id, resposta, checked: check }) => {
				return {
					id,
					resposta,
					checked: check,
		...
		newRespostas[idResposta].checked = checked;
		...
		<>
			<Body>
				{!isLoading && (
					<>
						<ProgressWrapper>
							<ProgressBar progresso={((step + 1) * 100) / arrayQuestionario.length} animated={isAnimated} />
							<SpanProgress>
						...
						<DivRespostas>
							{arrayQuestionario[step].respostas.map(({ resposta, id }, index) => {
								return (
									<FormControlLabel
										key={id}
										control={
											<Checkbox
												id={id}
												checked={respostasQuestao[index].checked}
						...
						<ButtonWrapper>
							<Buttons titulo='Voltar' voltar='true' onClick={handleVoltar} disabled={step < 1} />
							<Buttons
								titulo='Próximo'
